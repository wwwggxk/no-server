...
function streamThrough(callback) {
...
var Server = (function () {
    ...
    function S(root, options) {
        var _server, _wsServer, wsClient, self = this, isAbsolute, isFromIndex,
            relativePath,
            defaults = {
                root: '.',
                host: 'http://localhost',
        ...
        if (!isAbsolute) {
            defaults.root = path.join(path.dirname(relativeFile), defaults.root);
        }
        ...
        this.start = function () {
            ...
            isPortAvailable(self.options.port).then(function () {
                ...
                _server.on('listening', function () {
                    ...
                    console.log(date +
                            ' http server started: http://127.0.0.1:' +
                            self.options.port);
                    console.log(date +
        ...
        this.streamReloadCss = function () {
            return streamThrough(this.reloadCss.bind(this));
        };

        this.streamReloadAll = function () {
            return streamThrough(this.reloadAll.bind(this));
        };
    }
    ...
    S.prototype.processProxy = function (req, res) {
        ...
        req.on('end', function () {
            var options, agent, current, request, parsedUrl, key,
                allowedHeaders = [
                    'accept',
                    'accept-encoding',
                    'cache-control',
            ...
            for(key in req.headers) {
                if (allowedHeaders.indexOf(key.toLowerCase()) > -1) {
...
module.exports = function(exeFile) {
    var servers = [];
    relativeFile = exeFile;
    return {
        exec: function (command) {
            ...
            if (command.length) {
                var bin = command[0];
                var child = childProcess.spawn(bin, command.slice(1));
                console.log(command);
        ...
        streamReloadAll: function () {
            var self = this;
            return streamThrough(function () {
                self.reloadAll();
            });
        },
        streamReloadCss: function () {
            var self = this;
            return streamThrough(function () {
                self.reloadCss();
            });
